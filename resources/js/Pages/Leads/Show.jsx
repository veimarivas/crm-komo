...
export default function Show({ lead, stages, events, tasks, notes, members, contacts, companies, allTags, customFields, customValues, whatsappEnabled }) {
    ...
    const saveEdit = (e) => {
        e.preventDefault();
        editForm.patch(route('leads.update', lead.id), { preserveScroll: true });
    };
                    ...
                    <div className="min-w-0">
                        ...
                        {lead.source_ref && (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 mt-2 rounded-full text-xs font-bold ring-1 bg-blue-50 text-blue-800 ring-blue-200">
                                ...
                                {lead.source_url && (
                                    <a href={lead.source_url} target="_blank" rel="noreferrer" className="underline hover:text-blue-600 font-semibold">
                                        ver anuncio ↗
                                    </a>
                                )}
                            </span>
                ...
                <div className="grid gap-6 lg:grid-cols-3">
                    ...
                    <div className="space-y-6">
                        <form onSubmit={saveEdit} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 space-y-4">
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`rounded-full px-2.5 py-1 text-xs font-medium transition-all ${
                                                    active ? 'text-white shadow-md' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                                                }`}
                                    ...
                                    {newTag === null ? (
                                        <button
                                            type="button"
                                            onClick={() => setNewTag('')}
                                            className="rounded-full px-2.5 py-1 text-xs font-medium border border-dashed border-gray-300 text-gray-400 hover:border-emerald-400 hover:text-emerald-600 transition-all"
                                        >
                                            + Nueva
                                        </button>
                                    ) : (
                                            ...
                                            <input
                                                ...
                                                onKeyDown={(e) => {
                                                    if (e.key === 'Enter') {
                                                        e.preventDefault();
                                                        if (newTag.trim()) {
                                                            router.post(route('tags.store'), { name: newTag.trim() }, { preserveScroll: true, onSuccess: () => setNewTag(null) });
                                                        }
                                                    }
                            ...
                            <button
                                type="button"
                                onClick={() => { if (confirm('¿Eliminar este lead y su historial?')) router.delete(route('leads.destroy', lead.id)); }}
                                className="w-full text-xs text-red-500 hover:text-red-700 font-medium"
                            >
                                Eliminar lead
                            </button>
                        </form>
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                            <h3 className="text-sm font-bold text-gray-900 mb-1 flex items-center gap-2">
                                <span className="w-6 h-6 rounded-lg bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-white text-xs">💬</span>
                                WhatsApp
                            </h3>
                            {whatsappEnabled ? (
                                <form
                                    onSubmit={(e) => {
                                        e.preventDefault();
                                        waForm.post(route('leads.whatsapp', lead.id), { preserveScroll: true, onSuccess: () => waForm.reset() });
                                    }}
                                ...
                                >
                                    <textarea
                                        ...
                                        placeholder={`Mensaje para ${lead.contact?.name ?? lead.contact?.phone}…`}
                                        required
                                        className={inputClass}
                                    />
                                    {waForm.errors.text && <p className="text-xs text-red-500 font-medium">{waForm.errors.text}</p>}
                                    <button type="submit" disabled={waForm.processing} className="w-full px-4 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-emerald-600 to-teal-600 rounded-xl hover:from-emerald-500 hover:to-teal-500 disabled:opacity-50 transition-all shadow-lg shadow-emerald-500/20">
                                        {waForm.processing ? 'Enviando…' : 'Enviar WhatsApp'}
                                    </button>
                                </form>
                            ) : (
                            ...
                            {tab === 'tasks' && (
                                <div className="space-y-4">
                                    <form
                                        onSubmit={(e) => {
                                            e.preventDefault();
                                            taskForm.post(route('tasks.store'), { preserveScroll: true, onSuccess: () => taskForm.reset('text', 'due_at') });
                                        }}
                            ...
                            {tab === 'notes' && (
                                <div className="space-y-4">
                                    <form
                                        onSubmit={(e) => {
                                            e.preventDefault();
                                            noteForm.post(route('leads.notes.add', lead.id), { preserveScroll: true, onSuccess: () => noteForm.reset() });
                                        }}
                                        ...
                                        {notes.map((note) => (
                                            <li key={note.id} className="rounded-xl bg-amber-50/60 border border-amber-100 p-3.5">
                                                ...
                                                <p className="text-[11px] text-gray-400 mt-1.5">
                                                    {note.author?.name ?? '—'} · {new Date(note.created_at).toLocaleString('es', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                                                </p>
                                            </li>
                                        ))}
